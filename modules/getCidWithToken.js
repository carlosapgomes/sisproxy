/**
 * Created by carlos on 10/02/2021.
 */
// helper libraries
function getCidWithToken(req, res, pgClient, sodium, logger) {
  // check header
  if (
  ...
  if (
    ...
    const q = "SELECT * , ts_rank_cd(docvectors, query) AS rank FROM cid, websearch_to_tsquery('Portuguese', $1) query  WHERE query @@ docvectors order by rank DESC";
    return pgClient
      .query(q, values)
...
exports.getCidWithToken = getCidWithToken;
